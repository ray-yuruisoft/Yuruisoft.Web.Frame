...
namespace Yuruisoft.RS.Web.Controllers
{
    public class MathtoolController : Controller
    {
        ...
        public ActionResult SourceBuyPay(int id,string yuruisoft_session,string Email)//资源购买微信支付
        {
            ...
            if (yuruisoft_session != null)
            {
                ...
                if (CurrentPay != null)
                {
                    ...
                    return Json(WxExtensionClass.Wxpay(CurrentPay.WxBody, CurrentPay.TotalFee.ToString(), "http://lifark.com/notify_url.aspx", userinfo.appid, wx_mch_id, userinfo.openId, key));
                }
            }
            return Json(new
            {
        ...
        public static class WxExtensionClass//扩展函数集，主要针对登录和微信支付的，可加密为动态库
        {
            ...
            /// <returns></returns>
            public static string DecodeUserInfo(string raw, string signature, string encryptedData, string iv, string session_key)
            {
